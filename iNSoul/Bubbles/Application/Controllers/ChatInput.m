...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        // Set Defaults
        _maxY = [[NSNumber numberWithInt:60] retain]; // A frame origin y of 60 will prevent further expansion
        ...
        _sendBtnActiveColor = [[UIColor colorWithRed:0.142954 green:0.60323 blue:0.862548 alpha:1] retain];
        _sendBtnInactiveColor = [[UIColor lightGrayColor] retain];
...
- (void) textViewDidChange:(UITextView *)textView {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(chatInputDidChange:)])
        [self.delegate chatInputDidChange:textView.text];
}
